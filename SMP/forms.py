...
class SMPForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['Name'] = forms.CharField(max_length=15000)
        self.fields['Mentors'] = forms.CharField(max_length=15000)
        self.fields['Overview'] = forms.CharField(max_length=40000)
        self.fields['Platform'] = forms.CharField(max_length=10000)

        self.fields['exercise_freq'] = forms.IntegerField(
            initial=1, widget=forms.HiddenInput())
        for i in range(10):
            self.fields['Exercise_' +
                        str(i+1)] = forms.CharField(max_length=20000, required=False)
            if i is not 0:
        ...
        for i in range(10):
            self.fields['Course-content_' +
                        str(i+1)] = forms.CharField(max_length=10000, required=False)
            if i is not 0:
                self.fields['Course-content_' +
                            str(i+1)].widget = forms.HiddenInput()
        ...
        for i in range(10):
            ...
            for j in range(10):
                self.fields['Week_'+str(i+1)+'_Description_' +
                            str(j+1)] = forms.CharField(max_length=30000, required=False)
